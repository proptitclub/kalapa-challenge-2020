...
def  norm_field_36(df):
    #Nhóm do NLĐ và người SDLĐ đóng: 0
    n = [['DN','HX','CH','NN','TK','HC','XK']] #nhom 1
    n.append(['HT','TB','NO','CT','XB','TN','CS']) #nhom2
    n.append( ['QN','CA','CY','XN','MS','CC','CK','CB','HD','TE','BT',
          'HN','DT','DK','XD','TS','TC','TQ','TV','TA','TY','HG','LS',
          'PV'])

    n.append(['CN','HS','SV','GB'])
    n.append(['GD']) # cac nhom khac va GD

    def custom_x(x):
      if  not isinstance(x,str) and (np.isnan(x) or x == c.missing_value):
        return c.missing_value
      for step,x_i in enumerate(n):
        if x in x_i:
          return step + 1
      return 5

    df['Field_36'] = df['Field_36'].apply(lambda x:custom_x(x))
    return df

def norm_field_46(x):
    x = x.lower()

    for regex, replace in c.list_46.items():
        for text in regex:
            if x.find(text) != -1:
                return replace
    return c.missing_value
...
def preprocess_train_data(data_train_path, output_path):
    ...
    df_train = norm_field_36(df_train)
...
def preprocess_test_data(data_test_path, output_path):
    ...
    # norm field 36
    df_test = norm_field_36(df_test)
    df_test['Field_46'] = df_test['Field_46'].apply(lambda x: remove_accent(x))
    df_test["Field_46"] = df_test["Field_46"].apply(lambda x: norm_field_46(x))
